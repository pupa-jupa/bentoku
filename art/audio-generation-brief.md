...
Событие: одна ячейка мягко подсвечивается примерно на 4 секунды. Для Explain и Nudge достаточно `note_open`; отдельный сигнал нужен именно для появления полезного свечения.
...
- После проверки внутри игры эффекты намеренно сведены заметно тише первоначального ориентира: частые действия примерно −22…−17 dBFS, неверное решение около −15.5 dBFS, победа около −13 dBFS.
...
- `AudioService` использует девять коротких mono WAV-эффектов из `public/assets/sfx`; исходники воспроизводимо синтезируются командой `pnpm sfx:build`.
- На одно нажатие или начало drag запускается только один `piece_pick`, без прежнего многократного наложения.
- `success` начинается через 100 мс после последнего `piece_drop`, а `board_incorrect` — через 120 мс, чтобы сначала читалась укладка фигурки, затем итоговая обратная связь.
