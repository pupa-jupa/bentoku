...
export class ClueView extends Phaser.GameObjects.Container {
  ...
  constructor(scene: Phaser.Scene, clue: CluePattern, x: number, y: number, maxWidth: number) {
    super(scene, x, y);
    this.clue = clue;
    ...
    const padTop = isAnchor ? 38 : 34;
    const cardWidth = maxWidth;
    const cardHeight = gridHeight + padTop + 14;
    ...
    const title = scene.add
      .text(0, -cardHeight / 2 + 14, isAnchor ? 'CAFÉ MAP · FIXED' : clue.name.toUpperCase(), {
        fontFamily: isAnchor ? FONT_DISPLAY : FONT_BODY,
        fontSize: isAnchor ? '14px' : '11px',
        fontStyle: 'bold',
        color: '#6d5048',
        letterSpacing: 1,
      })
    ...
    for (const cell of clue.cells) {
      ...
      if (cell.animal && cell.food) {
        ...
        fitInside(animal, cellSize * 0.46);
        fitInside(food, cellSize * 0.42);
